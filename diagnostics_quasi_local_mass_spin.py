...
def register_CFunction_diagnostics_quasi_local_mass_spin(
    ...
    for i in range(3):
        for j in range(3):
            s_norm_sq += gammaUU[i][j] * s_d[i] * s_d[j]

    sU = ixp.zerorank1(dimension=3)
    for i in range(3):
        for j in range(3):
            sU[i] += gammaUU[i][j] * s_d[j]
    for i in range(3):
        sU[i] /= sp.sqrt(s_norm_sq)

    # B) Compute the rotational 1-form, omega_A = s^j * K_{jA}
    # where A is a surface index (0 for theta, 1 for phi).
    # The components of KDD are already in the spherical basis (r, theta, phi).
    omega_d = ixp.zerorank1(dimension=2)
    for A in range(2):
        for j in range(3):
            omega_d[A] += sU[j] * KDD[j][A + 1]
